...
public class talk1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        showText = GameObject.Find("Canvas").transform.Find("talk screen").GetComponent<Image>();
        // 캔버스 오브젝트 아래 자식 오브젝트로 있는 talkScreen을 호출합니다.

        charactors = new Image[2];  // 이미지 호출할 배열을 만듭니다.
        charactors[0] = GameObject.Find("Canvas").transform.Find("동구라").GetComponent<Image>();
        charactors[1] = GameObject.Find("Canvas").transform.Find("무룩").GetComponent<Image>();
        // 각 배열에 오브젝트를 연결합니다.
